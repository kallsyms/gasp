...
main {
   ...
   prompt {
      ...
      let result = parser.parse_wail_file(input);
      assert!(result.is_ok(), "Failed to parse ideal.wail: {:?}", result.err());
      ...
      assert!(template.prompt_template.contains("{{#each conversation.messages}}"));
      assert!(template.prompt_template.contains("{{this.role}}: {{this.content}}"));
      assert!(template.prompt_template.contains("{{return_type}}"));
